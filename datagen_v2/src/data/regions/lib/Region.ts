export class RegionTerrain {
    ...
    constructor(
        ...
        if (opts.amplitudeMin !== undefined)
            this.amplitude_min = opts.amplitudeMin;
        if (opts.amplitudeMax !== undefined)
            this.amplitude_max = opts.amplitudeMax;
...
export class RegionCaveBiome {
    ...
    constructor(
        ...
        if (opts.noiseThreshold !== undefined)
            this.noise_threshold = opts.noiseThreshold;
        if (opts.noiseScale !== undefined) this.noise_scale = opts.noiseScale;
...
export class RegionBiome {
    ...
    constructor(
        id: string,
        weight: number = 1,
        terrainPreference: TerrainPreference = "any",
    ) {
        this.id = id;
        this.weight = weight;
...
export class Region {
    ...
    constructor(
        ...
        this.cave_biome_default =
            opts.caveBiomeDefault ?? "phantasia:cave/chasm";
        if (opts.caveBiomes) this.cave_biomes = opts.caveBiomes;
        if (opts.terrain) this.terrain = opts.terrain;
        if (opts.biomeNoiseScale !== undefined)
            this.biome_noise_scale = opts.biomeNoiseScale;
    }
}
