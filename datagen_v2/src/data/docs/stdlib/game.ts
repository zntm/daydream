...
export const game = new Doc("Game API")
    ...
    .function("spawn_creature", "x, y, creature_id, variant?", "Creature?", "Spawns a creature at the specified tile position.", [["`x`", "number", "X position in tiles"], ["`y`", "number", "Y position in tiles"], ["`creature_id`", "string", "Creature ID or bare name"], ["`variant`", "string", "Optional creature variant"]])
