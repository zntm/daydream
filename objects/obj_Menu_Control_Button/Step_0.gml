...
for (var i = 0; i < _number; ++i)
{
    ...
    {
        if (_menu_layer != menu_layer) || !(boolean & (MENU_BUTTON_BOOL.IS_SELECTED | MENU_BUTTON_BOOL.IS_HOLDING)) continue;
    ...
    if (!_has_selected)
    {
        with (obj_Menu_Button)
        {
            if (index != i) || (boolean & MENU_BUTTON_BOOL.IS_HOLDING) continue;
...
with (obj_Menu_Button)
{
    if (boolean & MENU_BUTTON_BOOL.IS_HOLDING) && (on_select_hold != undefined)
    {
        on_select_hold();
    }
}
...
if (mouse_check_button_released(mb_left))
{
    ...
    with (obj_Menu_Dropdown)
    {
        if (boolean & MENU_BUTTON_BOOL.IS_SELECTED) && (!point_in_rectangle(mouse_x, mouse_y, bbox_left, bbox_top, bbox_right, bbox_bottom))
        {
            boolean ^= MENU_BUTTON_BOOL.IS_SELECTED;
...
if (mouse_check_button_pressed(mb_left))
{
    with (obj_Menu_Textbox)
    {
        if (!point_in_rectangle(mouse_x, mouse_y, bbox_left, bbox_top, bbox_right, bbox_bottom)) && (boolean & MENU_BUTTON_BOOL.IS_SELECTED)
        {
            sfx_play("phantasia:sfx/menu/button/deselect", global.settings.audio_ui);
