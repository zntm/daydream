    ...
    if (surface_refresh & SURFACE_REFRESH_BOOL.LIGHTING)
    {
        ...
        for (var i = 0; i < chunk_in_view_length; ++i)
        {
            ...
            while (l < CHUNK_SIZE)
            {
                ...
                /* skip row entirely if all bits are set */
                if (_data != (1 << CHUNK_SIZE) - 1)
                {
                    ...
                    for (var m = 0; m < CHUNK_SIZE; ++m)
                    {
                        if !(_data & (1 << m))
                        {
                            draw_sprite_ext(spr_Light, 0, _x2, _padding_offset + m, _xscale, 1, 0, c_white, 1);
                        }
                    }
                }
        ...
        else if (surface_get_width(surface_lighting) != _surface_lighting_width) || (surface_get_height(surface_lighting) != _surface_lighting_height)
        {
        	surface_resize(surface_lighting, _surface_lighting_width, _surface_lighting_height);
        }
