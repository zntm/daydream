...
enum PROG_VM
{
    ...
    FRAME_RET_SP,   /* array (real: stack slot for the return value) */
    FRAME_THIS,     /* array (any: saved receiver) */
    FRAME_CLASS,    /* array (struct: saved active class) */
...
function proglang_vm_reset(_vm)
{
    ...
    if (array_length(_vm[PROG_VM.FRAME_RET_SP]) < 1_000)
    {
        _vm[@ PROG_VM.FRAME_RET_SP] = array_create(1_000);
    }

    if (array_length(_vm[PROG_VM.FRAME_THIS]) < 1_000)
    {
        _vm[@ PROG_VM.FRAME_THIS] = array_create(1_000);
    }

    if (array_length(_vm[PROG_VM.FRAME_CLASS]) < 1_000)
    {
        _vm[@ PROG_VM.FRAME_CLASS] = array_create(1_000);
    }
...
function proglang_vm_run(_vm, _entry_bytecode)
{
    ...
    var _f_ret_sp = _vm[PROG_VM.FRAME_RET_SP];
    var _f_this = _vm[PROG_VM.FRAME_THIS];
    var _f_class = _vm[PROG_VM.FRAME_CLASS];
                ...
                switch (_op)
                {
                    ...
                    case PROG_OP.CALL_SPREAD:
                        ...
                        if (is_array(_val)) && (array_length(_val) >= PROG_CLOSURE.SIZE) && (_val[PROG_CLOSURE.TYPE] == "closure")
                        {
                            ...
                            _f_gref[@ _fp] = _gref;
                            _f_ret_sp[@ _fp] = _callee_index;
                            _f_this[@ _fp] = _vm[PROG_VM.CURRENT_THIS];
                            _f_class[@ _fp] = _vm[PROG_VM.ACTIVE_CLASS];
                            ...
                            _vm[@ PROG_VM.CURRENT_THIS] = _val[PROG_CLOSURE.RECEIVER];
                            _vm[@ PROG_VM.ACTIVE_CLASS] = _val[PROG_CLOSURE.DEFINING_CLASS];
                        }
                        else if (is_struct(_val)) && (struct_exists(_val, "__super__"))
                        {
                            var _super_class = _val.__super__;

                            if (!is_struct(_super_class)) || (_super_class[$ "__type__"] != "class")
                            {
                                runtime_error(PROGLANG_ERROR_TYPE.SUPER_ERROR, "Invalid super call target.");
                            }

                            if (!struct_exists(_super_class, "constructor_code")) || (_super_class.constructor_code == undefined)
                            {
                                runtime_error(PROGLANG_ERROR_TYPE.SUPER_ERROR, $"Super class '{_super_class.name}' has no constructor.");
                            }
                            ...
                            _curr_bytecode = _super_class.constructor_code;
                            _code = _curr_bytecode.code;
                            _constants = _curr_bytecode.constants;
                            _length = array_length(_code);
                            _ip = 0;
                            ...
                            var _super_scope = array_create(PROG_SCOPE.SIZE);

                            _super_scope[@ PROG_SCOPE.VARS] = {}
                            _super_scope[@ PROG_SCOPE.PARENT] = undefined;
                            _super_scope[@ PROG_SCOPE.TRACKED_RESOURCES] = [];

                            _vm[@ PROG_VM.SCOPE] = _super_scope;
                            _scope = _super_scope;
                            _vm[@ PROG_VM.CURRENT_THIS] = _val.receiver;
                            _vm[@ PROG_VM.ACTIVE_CLASS] = _super_class;
                        }
                    ...
                    case PROG_OP.RETURN:
                        ...
                        _vm[@ PROG_VM.SCOPE] = _scope;
                        _vm[@ PROG_VM.GLOBAL_REF] = _gref;
                        _vm[@ PROG_VM.CURRENT_THIS] = _f_this[_fp];
                        _vm[@ PROG_VM.ACTIVE_CLASS] = _f_class[_fp];
                    ...
                    case PROG_OP.MAKE_CLOSURE:
                        ...
                        if (is_array(_func)) && (array_length(_func) >= PROG_FUNC.SIZE)
                        {
                            ...
                            _closure_arr[@ PROG_CLOSURE.DEFINING_CLASS] = _vm[PROG_VM.ACTIVE_CLASS];
                            _closure_arr[@ PROG_CLOSURE.RECEIVER] = _vm[PROG_VM.CURRENT_THIS];
                    ...
                    case PROG_OP.NEW_INSTANCE:
                        ...
                        if (struct_exists(_class, "constructor_code") && (_class.constructor_code != undefined))
                        {
                            ...
                            _vm[@ PROG_VM.ACTIVE_CLASS] = _class;
                    ...
                    case PROG_OP.CLASS_DEF:
                        var _class_def = _constants[_arg];

                        if (is_struct(_class_def)) && (_class_def[$ "__type__"] == "class")
                        {
                            if (is_string(_class_def.super_class))
                            {
                                var _super_name = _class_def.super_class;
                                var _resolved_super = undefined;
                                var _class_scope = _scope;

                                while (_class_scope != undefined)
                                {
                                    if (struct_exists(_class_scope[PROG_SCOPE.VARS], _super_name))
                                    {
                                        var _candidate = _class_scope[PROG_SCOPE.VARS][$ _super_name];

                                        if (is_struct(_candidate)) && (_candidate[$ "__type__"] == "class")
                                        {
                                            _resolved_super = _candidate;
                                            break;
                                        }
                                    }

                                    _class_scope = _class_scope[PROG_SCOPE.PARENT];
                                }

                                if (_resolved_super == undefined) && (struct_exists(_vm[PROG_VM.CLASS_REGISTRY], _super_name))
                                {
                                    _resolved_super = _vm[PROG_VM.CLASS_REGISTRY][$ _super_name];
                                }

                                if (_resolved_super == undefined) && (struct_exists(_gref, _super_name))
                                {
                                    var _global_candidate = _gref[$ _super_name];

                                    if (is_struct(_global_candidate)) && (_global_candidate[$ "__type__"] == "class")
                                    {
                                        _resolved_super = _global_candidate;
                                    }
                                }

                                if (_resolved_super == undefined)
                                {
                                    runtime_error(PROGLANG_ERROR_TYPE.SUPER_ERROR, $"Superclass '{_super_name}' not found.");
                                }

                                _class_def.super_class = _resolved_super;
                            }

                            if (_class_def.name != undefined)
                            {
                                _vm[@ PROG_VM.CLASS_REGISTRY][$ _class_def.name] = _class_def;
                            }
                        }

                        _stack[@ _sp++] = _class_def;
                        break;
...
function proglang_vm_create_impl()
{
    ...
    _vm[@ PROG_VM.FRAME_RET_SP] = array_create(256);
    _vm[@ PROG_VM.FRAME_THIS] = array_create(256);
    _vm[@ PROG_VM.FRAME_CLASS] = array_create(256);
