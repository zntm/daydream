...
function ProgParser(_tokens) constructor
{
    ...
    {
        var _imports = [];
        var _uses_braces = match(PROG_TOKEN.LBRACE);

        /* Named imports: import a, b from "path" or import { a, b } from "path" */
        do
        {
        ...
        if (_uses_braces)
        {
            consume(PROG_TOKEN.RBRACE, "Expected '}' after imported names.");
        }
        ...
        while (!check(PROG_TOKEN.RBRACE) && !is_at_end())
        {
            ...
            if (match(PROG_TOKEN.FN) || check(PROG_TOKEN.VAR) || check(PROG_TOKEN.IDENTIFIER))
            {
                ...
                if (match(PROG_TOKEN.FN))
                {
                    ...
                    {
                        var _method_name = consume(PROG_TOKEN.IDENTIFIER, "Expected function name.").lexeme;
                        ...
                        _decl = new ProgASTFuncDecl(_method_name, _params, _body, false);
                        _is_method = true;
                    }
                    else
                    {
