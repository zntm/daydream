function proglang_execute_native(_bytecode, _context = {}, _filepath = "") {
    ...
    // 3. Inject Context & Filepath
    if (_filepath != "") {
        var _dirname = proglang_get_directory(_filepath);
        proglang_vm_define_global_string(_handle, "__dirname", _dirname);
        proglang_vm_define_global_string(_handle, "__filename", _filepath);
    }

    var _ctx_names = variable_struct_get_names(_context);
    for (var i = 0; i < array_length(_ctx_names); i++) {
        var _name = _ctx_names[i];
        var _val = _context[$ _name];
    ...
    // 5. Execution Loop with Yield Handling
    ...
    while (_status == 2) { // YIELD_EXTERNAL_CALL
        ...
        for (var i = _arg_count - 1; i >= 0; i--) {
            var _type = proglang_vm_get_type(_handle);
            if (_type == 1) _args[i] = proglang_vm_pop_string(_handle); // String
            else _args[i] = proglang_vm_pop_double(_handle); // Real/Bool/Nil
        }
        proglang_vm_pop_double(_handle); // Pop callee itself

        // Execute Call in GML
        var _result = undefined;
        if (_callee_name == "__import__") {
            _result = proglang_load_module(_args[0], _filepath);
        } else if (_callee_name == "__export_set__") {
            if (active_module != undefined) active_module.exports[$ _args[0]] = _args[1];
            _result = undefined;
        } else if (variable_struct_exists(global.proglang_functions, _callee_name)) {
    ...
    if (_status == 1 || _status == 0) {
        var _type = proglang_vm_get_type(_handle);
        if (_type == 1) _final_result = proglang_vm_pop_string(_handle);
        else _final_result = proglang_vm_pop_double(_handle);
    } else {
