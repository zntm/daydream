function draw_text_cuteify(_x, _y, _string, _xscale = 1, _yscale = 1, _angle = 0, _colour = c_white, _alpha = 1, _asset_prefix = "")
{
    ...
    for (var i = 0; i <= _line_count; ++i)
    {
        ...
        for (var j = 0; j < _node_count; ++j)
        {
            ...
            if (_type == CUTEIFY_NODE.COLOUR_POP)
            {
                _colour = _base_colour;
                continue;
            }
            ...
            if (_type == CUTEIFY_NODE.ITALIC) { _state_italic = true; continue; }
            if (_type == CUTEIFY_NODE.ITALIC_END) { _state_italic = false; continue; }
            ...
            if (_type == CUTEIFY_NODE.TEXT)
            {
                ...
                for (var l = 1; l <= _text_length; ++l)
                {
                    ...
                    var _italic_x = (_state_italic ? (0.35 * _yscale * _cos) : 0);
                    var _italic_y = (_state_italic ? (0.35 * _yscale * _sin) : 0);

                    draw_text_transformed_colour(
                        _draw_px + _italic_x, _draw_py + _italic_y,
                    ...
                    {
                        draw_text_transformed_colour(
                            _draw_px + _italic_x + (0.2 * _yscale * _cos_90),
                            _draw_py + _italic_y + (0.2 * _yscale * _sin_90),
                            _char, _xscale, _yscale, _angle,
                            _colour, _colour, _colour, _colour, _alpha * 0.8
                        );
                    }
                    ...
                    if (_state_bold)
                    {
                        ...
                        draw_text_transformed_colour(
                            _draw_px + _italic_x + (1 * _xscale * _cos), _draw_py + _italic_y + (1 * _xscale * _sin),
                            _char, _xscale, _yscale, _angle,
                            _colour, _colour, _colour, _colour, _alpha
                        );
                    }
                    ...
                    if (_draw_line)
                    {
                        var _lx1 = _draw_px + _italic_x + (_line_y_offset * _cos_90);
                        var _ly1 = _draw_py + _italic_y + (_line_y_offset * _sin_90);
