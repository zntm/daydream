...
function chunk_generate(_chunk)
{
    ...
    // Array is directly assigned from pool query
    ...
    for (var i = 0; i < CHUNK_SIZE; ++i)
    {
        ...
        for (var j = 0; j < CHUNK_SIZE; ++j)
        {
            ...
            if (_world_y >= _surface_height)
            {
                ...
                {
                    var _tile_base = worldgen_get_tile_base(_world_x, _world_y, _surface_biome, _cave_biome, _surface_height, _cave_bit & (1 << j), _world_seed, _world_data, _global_biome_data);
                ...
                {
                    var _tile_wall = worldgen_get_tile_wall(_world_x, _world_y, _surface_biome, _cave_biome, _surface_height, _world_seed, false, _world_data, _global_biome_data);
            ...
            if !(_skip_z & (1 << _z)) && (_world_y >= _surface_height - 1)
            {
                ...
                {
                    var _tile_base = worldgen_get_tile_base(_world_x, _world_y + 1, _surface_biome, _cave_biome_next, _surface_height, true, _world_seed, _world_data, _global_biome_data);
                    
                    var _tile_foliage = worldgen_get_tile_foliage(_world_x, _world_y, _surface_biome, _cave_biome, _tile_base, _surface_height, _world_seed, _global_biome_data);
