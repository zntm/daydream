...
function TerrainGenerator(_config = {}) constructor
{
    ...
    /// @desc Get detailed density for shaping (Minecraft-like)
    /// @param {Real} _x World X
    /// @param {Real} _y World Y
    /// @param {Real} _z_offset Z-layer offset (for walls/overhangs)
    /// @param {Struct} _world_data WorldData
    /// @param {Real} _seed World seed
    static get_density_detailed = function(_x, _y, _z_offset, _world_data, _seed)
    {
        // 1. Sample Control Parameters (C, E, PV) - 1D (X-based) or 2D (X, Y-based)?
        // For a side-scroller, "Continentalness" (Land vs Ocean) is usually X-based.
        // "Peaks & Valleys" is X-based.
        // "Erosion" is X-based.
        
        var _c_settings = _world_data.get_continentalness_noise();
        var _e_settings = _world_data.get_erosion_noise();
        var _pv_settings = _world_data.get_pv_noise();
        
        // Sample X-based noise for "Biomes/Inland" layout
        var _c = open_simplex_noise(_x * _c_settings.scale, _seed * 1.1, 1.0, _c_settings.octaves);
        var _e = open_simplex_noise(_x * _e_settings.scale, _seed * 2.2, 1.0, _e_settings.octaves);
        var _pv = open_simplex_noise(_x * _pv_settings.scale, _seed * 3.3, 1.0, _pv_settings.octaves);
        
        // 2. Evaluate Splines to get Base Height / Shape Factors
        var _s_c = _world_data.get_spline_continentalness();
        var _s_e = _world_data.get_spline_erosion();
        var _s_pv = _world_data.get_spline_pv();
        
        var _base_height = spline_evaluate(_s_c, _c);
        var _erosion_factor = spline_evaluate(_s_e, _e);
        var _pv_height = spline_evaluate(_s_pv, _pv);
        
        // Combine to get Target Surface Height at this X
        // Base + (PV * Erosion)
        var _target_height = _base_height + (_pv_height * _erosion_factor);
        
        // 3. Calculate Density
        // Density = (TargetHeight - Y) + Noise3D
        // If (TargetHeight - Y) is 0, we are at surface.
        // Positive = Solid (below surface), Negative = Air (above).
        var _height_term = (_target_height - _y);
        
        // Apply 3D Shaping Noise
        // Use "Z" parameter simulates 3D density noise (caves, overhangs)
        // We use Z-offset to differentiate Solid vs Wall layers.
        // Assuming open_simplex_noise is 2D, we can simulate Z by offsetting Y?
        // OR we can pass it if supported. Logic: Try to use Y-offset simulation for robustness.
        // "Slightly different z layer... 0 for solid, 0.1 for wall". 
        // Small offset implies correlation.
        
        var _shape_noise_scale = 0.02; // Hardcoded or from config?
        var _shape_noise = open_simplex_noise(
            _x * _shape_noise_scale, 
            (_y * _shape_noise_scale) + _z_offset, // Shift Y to simulate Z-slice
            1.0, 
            4
        );
        
        // Scale noise amplitude based on Erosion (smoother areas have less noise)
        // Erosion: -1 (smooth) -> 1 (jagged)
        // _erosion_factor is e.g. 0.2 to 2.5
        var _noise_amplitude = 40.0 * _erosion_factor; // 40 blocks variation
        
        var _final_density = _height_term + (_shape_noise * _noise_amplitude);
        
        // HARD LIMITS (Bedrock / Sky)
        // Force solid at bottom
        if (_y > _world_data.get_world_height() - 10) _final_density += 1000;
        // Force air at very top (optional)
        // if (_y < 100) _final_density -= 1000;
