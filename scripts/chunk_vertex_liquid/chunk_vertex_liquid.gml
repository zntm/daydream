function chunk_vertex_liquid(_buffer, _texel_width, _texel_height, _animation_type, _atla, _atla_sprite, _index, _x, _y, _xscale, _yscale, _rotation, _level = 8)
{
    ...
    // Calculate level ratio (1-8 -> 0.125 to 1.0)
    var _level_ratio = clamp(_level, 1, 8) / 8;
    
    // Crop the bottom of the UV based on level (keeping top constant)
    var _v1_cropped = lerp(_v0, _v1, _level_ratio);
    
    // Adjust height for cropped portion
    var _height_cropped = _height * _level_ratio;
    
    var _xoffset = -_xscale * (((_atla_value >> 0)  & 2047) - 1024);
    var _yoffset = -_yscale * (((_atla_value >> 11) & 2047) - 1024);
    
    // Adjust y offset to move sprite down when cropped (liquid settles to bottom)
    var _yoffset_cropped = _yoffset + (_height * (1 - _level_ratio) * _yscale);
    
    var _xw = (_xscale * _width)  + _xoffset;
    var _yh = (_yscale * _height_cropped) + _yoffset_cropped;
    ...
    var _c = _yoffset_cropped * _cos;
    var _d = _yoffset_cropped * _sin;
    ...
    vertex_texcoord(_buffer, _u0, _v1_cropped);
    vertex_float3(_buffer, (_number << 24) | _animation_type, _index, _width * _texel_width);
    ...
    vertex_texcoord(_buffer, _u1, _v1_cropped);
    vertex_float3(_buffer, (_number << 24) | _animation_type, _index, _width * _texel_width);
}
