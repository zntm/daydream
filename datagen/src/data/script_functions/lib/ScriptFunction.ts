...
export class ScriptFunction {
    ...
    setExample(example: string | string[]) {
    ...
    toVSCodeSnippet() {
        ...
        if (this.example) {
            description += `**Example:**\n\`\`\`\n${Array.isArray(this.example) ? this.example.join("\n") : this.example}\n\`\`\``;
        }
    ...
    toTypeDefinition() {
        ...
        if (this.example) {
                const ex = Array.isArray(this.example) ? this.example.join("\n * ") : this.example;
                jsdoc += ` * @example\n * ${ex}\n`;
        }
        ...
        return `${jsdoc}\ndeclare function ${this.name}(${params}): ${mapType(this.returnType)};\n`;
    }
}
